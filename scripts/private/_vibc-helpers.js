...
async function getIbcApp(network) {
	try {
		...
		console.log(
			`🗄️  Fetching IBC app on ${network} at address: ${ibcAppAddr}`
		);
...
function getDispatcherAddress(network) {
	...
	let dispatcherAddr;
	if (network === 'optimism') {
		dispatcherAddr = config.proofsEnabled
...
async function getDispatcher(network) {
	...
	try {
		if (network === 'optimism') {
			...
			const opDispatcherAbi = await fetchABI(
				explorerUrl,
				dispatcherAddress
			);
			dispatcher = new ethers.Contract(
				dispatcherAddress,
				opDispatcherAbi,
				providerOptimism
			);
			...
			const baseDispatcherAbi = await fetchABI(
				explorerUrl,
				dispatcherAddress
			);
			dispatcher = new ethers.Contract(
...
function getUcHandlerAddress(network) {
	...
	let ucHandlerAddr;
	if (network === 'optimism') {
		ucHandlerAddr = config.proofsEnabled
			? process.env.OP_UC_MW
			: process.env.OP_UC_MW_SIM;
	} else if (network === 'base') {
		ucHandlerAddr = config.proofsEnabled
			? process.env.BASE_UC_MW
			: process.env.BASE_UC_MW_SIM;
	} else {
		throw new Error('❌ Invalid network');
	}
...
async function getUcHandler(network) {
	...
	try {
		if (network === 'optimism') {
			explorerUrl = explorerOpUrl;
			ucHandlerAddress = config.proofsEnabled
				? process.env.OP_UC_MW
				: process.env.OP_UC_MW_SIM;

			const opUcHandlerAbi = await fetchABI(
				explorerUrl,
				ucHandlerAddress
			);
			ucHandler = new ethers.Contract(
				ucHandlerAddress,
				opUcHandlerAbi,
				providerOptimism
			);
			...
			const baseUcHandlerAbi = await fetchABI(
				explorerUrl,
				ucHandlerAddress
			);
			ucHandler = new ethers.Contract(
