...
function getConfigPath() {
	...
	const configRelativePath = process.env.CONFIG_PATH
...
async function fetchABI(explorerUrl, contractAddress) {
	try {
		const response = await axios.get(
			`${explorerUrl}api/v2/smart-contracts/${contractAddress}`,
			{
		...
		if (response.status === 200) {
			...
			console.error(
				`❌ Failed to fetch ABI, status code: ${response.status}`
			);
			return null;
		}
	} catch (error) {
...
function areAddressesEqual(address1, address2) {
	...
	if (!hre.ethers.isAddress(address1) || !hre.ethers.isAddress(address2)) {
		throw new Error(
			'❌ One or both addresses are not valid Ethereum addresses'
		);
	}
