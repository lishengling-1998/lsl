...
contract CustomChanIbcApp is IbcReceiverBase, IbcReceiver {
    ...
    function getConnectedChannels()
        external
        view
        returns (ChannelMapping[] memory)
    {
        return connectedChannels;
    }

    function updateSupportedVersions(
        string[] memory _supportedVersions
    ) external onlyOwner {
    ...
    function onRecvPacket(
    ...
    ) external virtual onlyIbcDispatcher returns (AckPacket memory ackPacket) {
        ...
        return
            AckPacket(
                true,
                abi.encodePacked(
                    '{ "account": "account", "reply": "got the message" }'
                )
            );
    }
        ...
        returns (string memory selectedVersion)
    {
        ...
        if (
            keccak256(abi.encodePacked(counterparty.version)) !=
            keccak256(abi.encodePacked(""))
        ) {
            require(
                keccak256(abi.encodePacked(counterparty.version)) ==
